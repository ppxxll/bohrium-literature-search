...
- **Bohrium 对话整理**：读取可访问的对话分享页、导出列表或页面内容，整理研究问题、带引用的综述和已获取的全部参考文献；记录页面声称篇数、实际获取篇数及逐篇核验状态。
...
```text
请用 bohrium-literature-search 整理这段 Bohrium 对话：<分享链接>。提取其研究方向分析、综述中引用的论文和参考文献面板的全部可访问条目；逐篇核对题录，并写一篇带编号引用的中文综述。列明页面声称篇数、实际获取篇数和未能核验的条目。
```

结果通常分中文、英文两组。每篇包含题目、作者、期刊或会议、年份、DOI、基于摘要或全文的简要介绍、与主题的关系及来源链接。数量、格式和筛选条件可在提问时指定。

对话整理需要可访问的分享链接、导出内容或足够完整的页面资料。单张截图只能提取可见内容；无法访问的分页、登录后内容和 Bohrium 内部搜索结果不会被当作已获取文献。它可重写并核验综述，不能保证复制 Bohrium 的内部检索过程或自动取得其全部语料。
