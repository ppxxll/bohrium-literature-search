...
- **Bohrium 对话整理**：优先利用可访问的批量引用导出，结合对话页面整理研究问题、综述与参考来源；分别统计正文引用、参考文献面板和全部搜索结果，逐条标注类型与核验状态。
- **论断追溯与导出**：把综述论断对应到引用和摘要或全文证据；长清单可保存为 Markdown/CSV，需要时生成仅含已确认字段的 BibTeX 或 RIS。
...
```text
请用 bohrium-literature-search 整理这段 Bohrium 对话：<分享链接或批量引用导出文件>。提取其研究方向分析、综述中引用的来源和参考文献面板的全部可访问条目；区分论文与其他来源，逐条核验，并写一篇带编号引用和证据对应表的中文综述。列明页面声称数量、实际获取数量和未能核验的条目，另给我 CSV 文献清单。
```

结果通常分中文、英文两组。每篇包含题目、作者、期刊或会议、年份、DOI、基于摘要或全文的简要介绍、与主题的关系及来源链接。数量、格式和筛选条件可在提问时指定。

对话整理需要可访问的分享链接、导出内容或足够完整的页面资料。单张截图只能提取可见内容；无法访问的分页、登录后内容和 Bohrium 内部搜索结果不会被当作已获取文献。它可重写并核验综述，不能保证复制 Bohrium 的内部检索过程或自动取得其全部语料。当前流程尚未用一份真实 Bohrium 批量导出完成端到端验收。
...
| [`references/bohrium-conversation.md`](references/bohrium-conversation.md) | Bohrium 对话、综述、引用和全部参考文献的提取与核验流程 |
